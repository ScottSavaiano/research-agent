...
**Last edited:** 2026-07-08 (Cowork — **teacher-admin critique applied** (`critiques/redirect-off-topic-research-critique-2026-07-08.md`; 0 blocking, 2 substantive, 4 minor): **S1** — the one stale term "core/push" → **core/extension** (the "push" → "extension" rename is program-wide since 2026-07-06). **S2 — RESOLVED (educator ruling 2026-07-08):** the whole diagnostic-read apparatus ("topic drift as information" / reading whether the student is "stuck" or "avoiding") is **removed** from both siblings, and the off-topic park moment is reworded to the clean **name-it-off-topic → park to the weekly reflection list → return to the research** form — no psychologizing of the student's motives. Sweep otherwise clean: no other retired terms, skill names current, STS eligibility correctly scoped to writing only, "Review Agent" capitalized, the weekly-reflection Tier-3 dependency correctly stated. Prior: 2026-06-29 (Cowork — first draft. Sibling of the committed project-mentor `redirect-off-topic` (the recognition heuristic, the X5 park policy, the never-pursue / if-pressed follow-through, the diagnostic register, the `check-budget` composition, and the bundling/protection model transfer directly). Research-specific: the scope-naming language (the research agent's lane = methodology in practice); the **three-outcome routing** (off-topic → park; research-but-a-sibling's-lane → route; in-lane → handle); and the deferral of writing-feedback requests to §8.2/X4. Authored against `design/cross-agent-obligations.md` X5/X4/R9/R1, the research-agent SOUL's "When you bring me something that isn't research work" section, and `decision-history-and-rationale.md` §13. Pairs with `check-budget` (re-bundled).))
...
- **The mentor's design lane** — choosing or revising the **research question**, identifying the **gap**, the **methodology design** decisions (the method-approach, the core/extension), the **synthesis** of the literature, and **whether to continue the project**. These go **back to the mentor** via the concrete handoff (the R1 return — *click the mentor's icon at the bottom-left of the app window*; typing `mentor` is the TUI equivalent).
...
**Off-topic (park):**
> "That's off-topic for your social-science research — your AP Bio essay. I'll park it in your weekly reflection list so you can take it up on your own later. For now, let's get back to your research — we were on your dataset."
